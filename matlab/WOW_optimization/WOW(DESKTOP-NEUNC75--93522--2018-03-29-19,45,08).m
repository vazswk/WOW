...
for fIndex = 1:3
    ...
    part_R_wow = R (120:140,240:260);
    %show_cost_dis(R)  
    % compute suitability
    xi{fIndex} = conv2(abs(R), rot90(abs(F{fIndex}), 2), 'same');
    cost_temp = (xi{fIndex}.^-1);
    cost_part_wow = cost_temp(120:140,240:260);
    save('part_R_wow','part_R_wow')
     save('part_cost_wow','cost_part_wow')
    ...
    cost{fIndex} = (xi{fIndex}.^p);
end
save('cost','cost')
% compute embedding costs \rho

rho = ( cost{1} + cost{2} + cost{3} ) .^ (-1/p);
show_cost_dis(cost{1}) 
show_cost_dis(cost{1}) 
show_cost_dis(cost{1}) 
show_cost_dis(rho) 
