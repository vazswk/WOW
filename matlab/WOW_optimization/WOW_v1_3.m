function [stego, distortion] = WOW_v1_3(cover, payload, params)
% -------------------------------------------------------------------------
...
for fIndex = 1:3
    ...
    cost{fIndex} = spatial_neighbourhood(xi{fIndex}.^p);
end

save('cost','cost')

%% cost 重新排序处理 :compute embedding costs \rho

B=1.5;
rhon = (cost{1} + cost{2} + cost{3}) .^ (-1/p);
[x, ind] = sort(rhon(:)); 
nn = numel(x);
rnds = raylrnd(B,[nn 1]);
[x1, ind1] = sort(rnds(:));
...
% % cost{1};
% rhon_1 = cost{1};
% [x, ind] = sort(rhon_1(:));  % 对熵的代价值做排序处理
% nn = numel(x);                
% rnds = raylrnd(B,[nn 1]);   % 生成一个锐利分布的随机数 rnds         
% [x1, ind1] = sort(rnds(:));  % 对rnds 重新排序得到x1 
% cost_new_1(ind) = x1;
% 
% % cost{2};
% rhon_2 = cost{2};
% [x, ind] = sort(rhon_2(:));  % 对熵的代价值做排序处理
% nn = numel(x);                
% rnds = raylrnd(B,[nn 1]);   % 生成一个锐利分布的随机数 rnds         
% [x2, ind2] = sort(rnds(:));  % 对rnds 重新排序得到x1 
% cost_new_2(ind) = x2;
% 
% 
% % cost{3};
% rhon_3 = cost{3};
% [x, ind] = sort(rhon_3(:));  % 对熵的代价值做排序处理
% nn = numel(x);                
% rnds = raylrnd(B,[nn 1]);   % 生成一个锐利分布的随机数 rnds         
% [x3, ind3] = sort(rnds(:));  % 对rnds 重新排序得到x1 
% cost_new_3(ind) = x3;
% RHO = (cost_new_1 + cost_new_2 + cost_new_3) .^ (-1/p);
% rho = reshape(RHO,sizeCover);
